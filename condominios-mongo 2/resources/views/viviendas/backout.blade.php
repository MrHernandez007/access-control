...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const idVivienda = urlSegments[urlSegments.length - 2]; // /viviendas/{id}/edit
  ...
  async function cargarVivienda() {
    try {
      ...
      console.log('Vivienda cargada:', vivienda);
      console.log('Estado cargado:', vivienda.estado);
      ...
      // Forzar a minúsculas si backend usa minúsculas para estado
      const estadoFormateado = vivienda.estado ? vivienda.estado.toLowerCase() : 'activo';
      document.getElementById('estado').value = estadoFormateado;

      await cargarResidentes(vivienda.residente?._id || null);
  ...
  document.getElementById('formEditarVivienda').addEventListener('submit', async (e) => {
    ...
    const data = {
      ...
      estado: document.getElementById('estado').value.toLowerCase()  // Aseguramos minúsculas
    };

    console.log('Datos a enviar:', data);

    try {
      const response = await fetch(`http://localhost:3000/api/viviendas/${idVivienda}`, {
        method: 'PUT',
        headers: {
